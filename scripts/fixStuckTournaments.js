...
// Run from project root: node scripts/fixStuckTournaments.js
...
// Fallback: Also try loading from root
if (!process.env.MONGO_URI) {
  dotenv.config({ path: path.join(__dirname, '..', '.env') });
}

// If still no MONGO_URI, use the hardcoded one from gameSeed.js as fallback
...
async function fixStuckTournaments() {
  try {
    ...
    const Tournament = (await import('../backEnd/schemas/TournamentSchema.js')).default;
    const tournamentService = (await import('../backEnd/services/tournamentService.js')).default;
